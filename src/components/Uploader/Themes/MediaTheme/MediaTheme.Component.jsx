...
import './MediaTheme.Style.scss';

export const MediaThemeComponent = memo(
  ({
    allFiles,
    defaultImage,
    uploadRef,
    ...
    return (
      <div
        className={`media-theme-component-wrapper ${
      ...
      >
        <div className="custom-dropzone-wrapper">
          <div className="dropzone-items-wrapper ">
            ...
            {allFiles
              && allFiles.map((file, index) => (
                <div
                  key={`${idRef}${index + 1}`}
                  className={`dropzone-item-wrapper ${
                    multiple ? 'multiple-items' : 'single-item'
                  }`}
                >
          ...
          {!isDisabled ? (
            <ButtonBase
              className="btns theme-solid mx-0 mt-1"
...
MediaThemeComponent.displayName = 'BoxThemeComponent';

MediaThemeComponent.propTypes = {
...
MediaThemeComponent.defaultProps = {
  allFiles: [],
  parentTranslationPath: '',
