...
import { MoreIcon, CornerDownIcon, CrossIcon } from '../../../icons';
...
export default function FieldTypeTab({
  ...
  const handleInputTypeChange = ({ target: { value } }) => {
    setInputType(value);
    const newField = fieldsItems[value];
    ...
    if (!isTitleInteracted)
      setLanguage((langs) => ({
        ...
        [langSaveTo]: {
          ...
          ...langs[langSaveTo],
          title: newField.languages[langSaveTo].title,
          value: '',
          options:
    ...
    if (!isPlaceholderIneteracted)
      setLanguage((langs) => ({
        ...
        ...(anotherLang && {
          [anotherLang]: {
            ...langs[anotherLang],
            placeholder: fieldsItems[value].languages[anotherLang].placeholder,
            options:
              langs[anotherLang].options
              || (newField.languages[anotherLang].options && [
                { ...newField.languages[anotherLang].options[0], id: newOptionID },
    ...
    () =>
      equation?.map((item, idx) => {
        if (
          ...
          return (
            <div className="d-flex-center my-2" key={`${item.type}-${idx}`}>
              <TextField
      ...
      >
        <InputOptionBox>
          ...
          <Switch
            ...
            sx={{ alignSelf: 'center', mr: 2.5 }}
            inputProps={{ 'aria-label': 'controlled-card-item-switch' }}
            disabled={cardItem.fillBy === 'recipient' || !isVisible}
          />
          <Typography variant="body13" align="center" color="dark.main">
            Active
          </Typography>
