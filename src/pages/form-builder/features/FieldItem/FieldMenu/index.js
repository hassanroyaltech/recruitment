  ...
  const handleSave = () => {
    if (equation?.length) {
      ...
      try {
        ...
        console.log(e);
        showError('Please write a valid equation!');
        return;
      }
    }
        ...
        items: data[containerId].items.map((item) =>
          item.id === cardId
            ? {
              ...
              isVisible,
              isVisibleFinalDoc,
              charMax: charMax ?? null, // Default for charMax
              charMin: charMin ?? null, // Default for charMin
              rowMin: rowMin ?? null, // Ensure null or value
              rowMax: rowMax ?? null,
              isRequired,
              disablePastDates,
              disableFutureDates,
