...
import { InfoIcon } from '../../../../../form-builder/icons';
...
export default function Panel({ templateData }) {
  const { t } = useTranslation(parentTranslationPath);
  const [isHintOpen, setIsHintOpen] = useState(true);
  const [vars, setVars] = useState([]);
  const getVarsHandler = useCallback(async () => {
    const response = await GetFormBuilderVars({
      is_flow: templateData?.code === DefaultFormsTypesEnum.Flows.key,
    });
