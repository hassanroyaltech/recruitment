...
import { TeamMembersSectionTabs } from '../shared/tabs-data';
import {
  DialogComponent,
  SwitchComponent,
  TabsComponent,
...
export const TeamMembersSectionDialog = ({
  language_uuid,
  language,
  sections,
  activeItem,
  ...
  const [teamMembersSectionTabsData] = useState(() => TeamMembersSectionTabs);
  ...
  const initStateRef = useRef({
    ...
    type: EvaBrandSectionsEnum.TeamMembersSection.key,
    order: 0,
    layout: 0,
    section_title: '',
    is_hidden: false,
        ...
        members: yup
          ...
          .of(
            yup.object().shape({
              ...
              // .required(t(`${translationPath}member-name-is-required`)),
  ...
  const onModeChangedHandler = (event, newValue) => {
    ...
    setState({
      ...
      value: {
        ...
        section_data: {
          ...state.section_data,
          members: state.section_data.members?.map((item) => ({
            ...item,
  ...
  const getIsAdvanced = useCallback(
    (localState) =>
      ...
      || localState.section_data.members.some(
  ...
  const saveHandler = async (event) => {
      ...
      section_data: {
        ...
        members:
          (state.section_data.members
            && state.section_data.members.map((item) => ({
              ...item,
    ...
    if (response && (response.status === 201 || response.status === 200)) {
      if (activeItem && activeItem.uuid)
        showSuccess(
          t(`${translationPath}team-members-section-updated-successfully`),
        );
      else
        showSuccess(
          t(`${translationPath}team-members-section-created-successfully`),
        );
      if (onSave) onSave();
    } else if (activeItem && activeItem.uuid)
      showError(t(`${translationPath}team-members-section-update-failed`), response);
    else
      showError(t(`${translationPath}team-members-section-create-failed`), response);
  };
  /**
  ...
  const editInit = useCallback(async () => {
    ...
    if (
      !localState.section_data.members
      || localState.section_data.members.length === 0
    )
      localState.section_data.members = [
        {
          name: '',
  ...
  return (
    <DialogComponent
      titleText={`${
        (activeItem
          && activeItem.uuid
          && t(`${translationPath}edit-team-members-section`))
        || t(`${translationPath}new-team-members-section`)
      ...
      dialogContent={
        <div
          className={`team-members-section-dialog-wrapper section-wrapper${
            (language.code === 'ar' && ' rtl-direction') || ''
          }`}
        >
          <SwitchComponent
            idRef="TeamMembersModeSwitchRef"
          ...
          <TabsComponent
            ...
            labelInput="label"
            idRef="TeamMembersSectionTabsRef"
            isWithLine
            isPrimary
...
TeamMembersSectionDialog.propTypes = {
...
TeamMembersSectionDialog.defaultProps = {
  activeItem: null,
  sections: [],
  translationPath: 'TeamMembersSectionDialog.',
};
