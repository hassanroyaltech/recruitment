...
import { SliderSectionTabs } from '../shared/tabs-data';
import {
  DialogComponent,
  SwitchComponent,
  TabsComponent,
...
// dialog to management the section of type slider
export const SliderSectionDialog = ({
  language_uuid,
  language,
  sections,
  activeItem,
  ...
  const [sliderSectionTabsData] = useState(() => SliderSectionTabs);
  ...
  const initStateRef = useRef({
    ...
    type: EvaBrandSectionsEnum.SliderSection.key,
    order: 0,
    layout: 0,
    section_title: '',
    is_hidden: false,
  ...
  const saveHandler = async (event) => {
    ...
    if (response && (response.status === 201 || response.status === 200)) {
      if (activeItem && activeItem.uuid)
        showSuccess(t(`${translationPath}slider-section-updated-successfully`));
      else showSuccess(t(`${translationPath}slider-section-created-successfully`));
      if (onSave) onSave();
    } else if (activeItem && activeItem.uuid)
      showError(t(`${translationPath}slider-section-update-failed`), response);
    else showError(t(`${translationPath}slider-section-create-failed`), response);
  };
  /**
  ...
  return (
    <DialogComponent
      titleText={`${
        (activeItem
          && activeItem.uuid
          && t(`${translationPath}edit-slider-section`))
        || t(`${translationPath}new-slider-section`)
      ...
      dialogContent={
        <div
          className={`slider-section-dialog-wrapper section-wrapper${
            (language.code === 'ar' && ' rtl-direction') || ''
          }`}
        >
          <SwitchComponent
            idRef="SliderModeSwitchRef"
          ...
          <TabsComponent
            data={sliderSectionTabsData}
            currentTab={activeTab}
            labelInput="label"
            idRef="SliderSectionTabsRef"
            isWithLine
            isPrimary
...
SliderSectionDialog.defaultProps = {
  activeItem: null,
  sections: [],
  translationPath: 'SliderSectionDialog.',
};
