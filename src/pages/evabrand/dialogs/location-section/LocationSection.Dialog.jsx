...
import { LocationSectionTabs } from '../shared/tabs-data';
import {
  DialogComponent,
  SwitchComponent,
  TabsComponent,
...
// dialog to management the section of type location
export const LocationSectionDialog = ({
  language_uuid,
  language,
  sections,
  activeItem,
  ...
  const [locationSectionTabsData] = useState(() => LocationSectionTabs);
  ...
  const initStateRef = useRef({
    ...
    type: EvaBrandSectionsEnum.LocationSection.key,
    order: 8,
    layout: 0,
    section_title: '',
    is_hidden: false,
  ...
  const schema = useRef(
    yup.object().shape({
      ...
      section_data: yup.object().shape({
        ...
        iframe_map: yup.string().nullable(),
        // .required(t(`${translationPath}please-select-location`)),
      }),
    }),
  );
  ...
  const saveHandler = async (event) => {
    ...
    /**
     * @Description this condition is to prevent submit directly after select
     * google map location from search & change focus to submit after select
     * the location search
     */
    if (
      document.activeElement
      && document.activeElement.id === 'googleMapSearchInputIdRef'
    ) {
    ...
    if (
      ...
      && errors['section_data.iframe_map'].error
    ) {
      showError(errors['section_data.iframe_map'].message);
      return;
    }
    ...
    if (response && (response.status === 201 || response.status === 200)) {
      if (activeItem && activeItem.uuid)
        showSuccess(t(`${translationPath}location-section-updated-successfully`));
      else showSuccess(t(`${translationPath}location-section-created-successfully`));
      if (onSave) onSave();
    } else if (activeItem && activeItem.uuid)
      showError(t(`${translationPath}location-section-update-failed`), response);
    else showError(t(`${translationPath}location-section-create-failed`), response);
  };
  /**
  ...
  return (
    <DialogComponent
      titleText={`${
        (activeItem
          && activeItem.uuid
          && t(`${translationPath}edit-location-section`))
        || t(`${translationPath}new-location-section`)
      ...
      dialogContent={
        <div
          className={`location-section-dialog-wrapper section-wrapper${
            (language.code === 'ar' && ' rtl-direction') || ''
          }`}
        >
          <SwitchComponent
            idRef="LocationModeSwitchRef"
          ...
          <TabsComponent
            data={locationSectionTabsData}
            currentTab={activeTab}
            labelInput="label"
            idRef="LocationSectionTabsRef"
            isWithLine
            isPrimary
...
LocationSectionDialog.propTypes = {
...
LocationSectionDialog.defaultProps = {
  activeItem: null,
  sections: [],
  translationPath: 'LocationSectionDialog.',
};
