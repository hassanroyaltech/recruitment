...
import { GallerySectionTabs } from '../shared/tabs-data';
import {
  DialogComponent,
  SwitchComponent,
  TabsComponent,
...
// dialog to management the section of type gallery
export const GallerySectionDialog = ({
  language_uuid,
  language,
  sections,
  activeItem,
  ...
  const [gallerySectionTabsData] = useState(() => GallerySectionTabs);
  ...
  const initStateRef = useRef({
    ...
    type: EvaBrandSectionsEnum.GallerySection.key,
    order: 0,
    layout: 0,
    section_title: '',
    is_hidden: false,
            ...
            yup.object().shape({
              ...
              media: yup
                ...
                .when(
                  ...
                  (value, fields) =>
                    (value
                      && fields.required(t(`${translationPath}image-is-required`)))
                    || fields,
                ),
  ...
  const onModeChangedHandler = (event, newValue) => {
    ...
    setState({
      ...
      value: {
        ...
        section_data: {
          ...state.section_data,
          medias: state.section_data.medias?.map((item) => ({
            ...item,
  ...
  const getIsAdvanced = useCallback(
    (localState) =>
      ...
      || localState.section_data.medias.some((item) => isHTML(item.title)),
    [],
  );
  ...
  const saveHandler = async (event) => {
    ...
    if (response && (response.status === 201 || response.status === 200)) {
      if (activeItem && activeItem.uuid)
        showSuccess(t(`${translationPath}gallery-section-updated-successfully`));
      else showSuccess(t(`${translationPath}gallery-section-created-successfully`));
      if (onSave) onSave();
    } else if (activeItem && activeItem.uuid)
      showError(t(`${translationPath}gallery-section-update-failed`), response);
    else showError(t(`${translationPath}gallery-section-create-failed`), response);
  };
  /**
  ...
  return (
    <DialogComponent
      titleText={`${
        (activeItem
          && activeItem.uuid
          && t(`${translationPath}edit-gallery-section`))
        || t(`${translationPath}new-gallery-section`)
      ...
      dialogContent={
        <div
          className={`gallery-section-dialog-wrapper section-wrapper${
            (language.code === 'ar' && ' rtl-direction') || ''
          }`}
        >
          <SwitchComponent
            idRef="GalleryModeSwitchRef"
          ...
          <TabsComponent
            data={gallerySectionTabsData}
            currentTab={activeTab}
            labelInput="label"
            idRef="GallerySectionTabsRef"
            isWithLine
            isPrimary
...
GallerySectionDialog.propTypes = {
...
GallerySectionDialog.defaultProps = {
  activeItem: null,
  sections: [],
  translationPath: 'GallerySectionDialog.',
};
