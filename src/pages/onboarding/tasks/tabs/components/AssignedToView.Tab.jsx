...
import { GetOnboardingTasksAssignedToView } from '../../../../../services/OnboardingTasks.Services';
...
export const AssignedToViewTab = ({
  ...
  const [assignedToView, setAssignedToView] = useState(() => ({
    results: [],
    totalCount: 0,
  }));
  ...
  const getOnboardingTasksAssignedToView = useCallback(async () => {
    if (isLoadingRef.current) return;
    isLoadingRef.current = true;
    setIsLoading(true);
    const response = await GetOnboardingTasksAssignedToView({
      ...filter,
  ...
  const generateFormLinkHandler = useCallback(
    async ({ row, member }) => {
      ...
      if (response && response.status === 200)
        GlobalHistory.push(
          `${response.data.results.form_link}&source=${NavigationSourcesEnum.OnboardingAssignToViewToFormBuilder.key}`,
        );
  ...
  useEffect(() => {
    getOnboardingTasksAssignedToView();
  }, [getOnboardingTasksAssignedToView]);

  // no need for scrolling since there is no pagination in the response
  ...
  //     onScrollHandler(assignedToView)
  //   }
  // }, [assignedToView, isLoading, onScrollHandler]);
    ...
    () => [
      {
        ...
        label: t('member'),
        isHidden: false,
        component: (row) => <span>{row.member}</span>,
      },
      {
        id: 2,
        input: 'section',
  ...
  return (
    <div className="pt-3 px-1">
      ...
      <Grid
        ...
        onScroll={onScrollHandler && onScrollHandler(assignedToView)}
        ref={bodyRef && bodyRef}
      >
        <Grid item md={12} className="pt-3" sx={{ maxWidth: '100%' }}>
          {assignedToView?.results?.map((item, index) => (
            <OnboardingAccordion
              actionComponent={
                item.redirect_link ? (
