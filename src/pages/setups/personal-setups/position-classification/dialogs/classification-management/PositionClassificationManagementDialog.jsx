...
import {
  CreateSetupsPositionClassifications,
  getSetupsPositionClassificationsById,
  UpdateSetupsPositionClassifications,
} from '../../../../../../services';
import { numericAndAlphabeticalAndSpecialExpression } from '../../../../../../utils';

export const PositionClassificationManagementDialog = ({
  activeItem,
  isOpen,
  onSave,
  isOpenChanged,
  ...
  const getEditInit = useCallback(async () => {
    setIsLoading(true);
    const response = await getSetupsPositionClassificationsById({
  ...
  const saveHandler = async (event) => {
    ...
    setIsLoading(true);
    let response;
    if (activeItem) response = await UpdateSetupsPositionClassifications(state);
    else response = await CreateSetupsPositionClassifications(state);
    setIsLoading(false);
    if (response && (response.status === 200 || response.status === 201)) {
      showSuccess(
        t(
          `${translationPath}${
            (activeItem && 'position-classification-updated-successfully')
            || 'position-classification-created-successfully'
          }`,
        ),
      );
      if (onSave) onSave();
      ...
      showError(
        t(
          `${translationPath}${
            (activeItem && 'position-classification-update-failed')
            || 'position-classification-create-failed'
          }`,
        ),
        response,
      );
  ...
  return (
    <DialogComponent
      maxWidth="md"
      titleText={(activeItem && 'edit-classification') || 'add-classification'}
      contentClasses="px-0"
      dialogContent={
...
PositionClassificationManagementDialog.propTypes = {
...
PositionClassificationManagementDialog.defaultProps = {
  isOpenChanged: undefined,
  onSave: undefined,
  activeItem: undefined,
  translationPath: 'PositionClassificationManagementDialog.',
};
