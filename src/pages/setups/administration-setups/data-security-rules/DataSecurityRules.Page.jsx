...
import { DataSecurityRulesManagementDialog } from './dialogs';
import { SystemActionsEnum, TablesNameEnum } from '../../../../enums';
import { DataSecurityRulesPermissions } from '../../../../permissions';

const translationPath = 'DataSecurityRulesPage.';
const parentTranslationPath = 'SetupsPage';

const DataSecurityRulesPage = () => {
  ...
  const [dataSecurityRules, setDataSecurityRules] = useState(() => ({
    results: [],
    totalCount: 0,
  }));
   ...
   * @Description this method is to get all data security rules with filter
   */
  const getAllDataSecurityRules = useCallback(async () => {
    setIsLoading(true);
    const response = await GetAllSetupsDataSecurityRules({ ...filter });
    setIsLoading(false);
    if (response && response.status === 200)
      setDataSecurityRules({
  ...
  useEffect(() => {
    getAllDataSecurityRules();
  }, [getAllDataSecurityRules, filter]);

  return (
    <div className="data-security-rules-page page-wrapper">
      <div className="page-header-wrapper px-3 pb-3">
        <span className="header-text-x2 d-flex mb-1">
          {t(`${translationPath}data-security-rules`)}
        </span>
        <span className="description-text">
          {t(`${translationPath}data-security-rules-description`)}
        </span>
      </div>
      <div className="page-body-wrapper">
          ...
          <div>
            ...
            <div className="d-inline-flex mb-2">
              <TableColumnsPopoverComponent
                ...
                feature_name={TablesNameEnum.DataSecurityRules.key}
            ...
            >
              ...
              <span className="px-1">
                {t(`${translationPath}add-data-security-rule`)}
              </span>
            </ButtonBase>
          </div>
        </div>
        <div className="px-2">
          <TablesComponent
            data={dataSecurityRules.results}
            isLoading={isLoading || isLoadingColumns}
            headerData={tableColumns}
            pageIndex={filter.page - 1}
            pageSize={filter.limit}
            totalItems={dataSecurityRules.totalCount}
            ...
            isSelectAllDisabled={
              !getIsAllowedPermissionV2({
                permissionId:
                  DataSecurityRulesPermissions.DeleteDataSecurityRules.key,
            ...
            onSelectAllCheckboxChanged={() => {
              setSelectedRows((items) =>
                globalSelectedRowsHandler(
                  items,
                  dataSecurityRules.results.filter(
      ...
      {isOpenManagementDialog && (
        <DataSecurityRulesManagementDialog
      ...
      {isOpenDeleteDialog && (
        <ConfirmDeleteDialog
          activeItem={activeItem}
          successMessage="data-security-rule-deleted-successfully"
          onSave={() => {
          ...
          descriptionMessage="data-security-rule-delete-description"
          deleteApi={DeleteSetupsDataSecurityRules}
          apiProps={{
            uuid:
              (isBulkDelete
          ...
          errorMessage="data-security-rule-delete-failed"
          activeItemKey="uuid"
          apiDeleteKey="uuid"
...
export default DataSecurityRulesPage;
