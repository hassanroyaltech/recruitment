...
import { DeleteSetupsPosition, GetAllSetupsPositions } from '../../../../services';
import {
...
import { PositionsManagementDialog } from './dialogs';
import { PositionsPermissions } from '../../../../permissions';
import { LookupImportDialog } from '../../shared/dialogs/lookups-import/LookupsImport.Dialog';

const translationPath = 'PositionsPage.';
...
const PositionsPage = () => {
   ...
   * @Description this method is to get all positions with filter
   */
  const getAllSetupsPositions = useCallback(async () => {
    setIsLoading(true);
    const response = await GetAllSetupsPositions({ ...filter });
    setIsLoading(false);
    ...
    <div className="positions-page page-wrapper">
      <div className="page-header-wrapper px-3 pb-3">
        <span className="header-text-x2 d-flex mb-1">
          {t(`${translationPath}positions`)}
        </span>
        <span className="description-text">
          {t(`${translationPath}positions-setup-description`)}
        </span>
      </div>
          ...
          <div className="px-2">
            ...
            <div className="d-inline-flex mb-2">
              <TableColumnsPopoverComponent
                ...
                feature_name={TablesNameEnum.Positions.key}
            ...
            >
              ...
              <span className="px-2">
                {t(`${translationPath}import-${LookupsImportEnum.position.value}`)}
              </span>
            </ButtonBase>
            <ButtonBase
              disabled={
                !getIsAllowedPermissionV2({
                  permissionId: PositionsPermissions.AddPosition.key,
                  permissions: permissionsReducer,
                })
              }
              onClick={() => {
            ...
            >
              ...
              <span className="px-1">{t(`${translationPath}add-position`)}</span>
            </ButtonBase>
          </div>
        </div>
        <div className="px-2">
          <TablesComponent
            ...
            totalItems={positions.totalCount}
            isWithCheckAll
            isWithCheck
            isDynamicDate
            uniqueKeyInput="uuid"
            isSelectAllDisabled={
              !getIsAllowedPermissionV2({
                permissionId: PositionsPermissions.DeletePosition.key,
                permissions: permissionsReducer,
              })
            }
            ...
            onSelectAllCheckboxChanged={() => {
              setSelectedRows((items) =>
                globalSelectedRowsHandler(
                  items,
                  positions.results.filter((item) => item.can_delete !== false),
                ),
              );
            }}
              ...
              getDisabledAction: (row, rowIndex, action) => {
                if (action.key === SystemActionsEnum.edit.key)
                  return !getIsAllowedPermissionV2({
                    permissionId: PositionsPermissions.UpdatePosition.key,
                    permissions: permissionsReducer,
                  });
      ...
      {isOpenLookupImportDialog && (
        <LookupImportDialog
          enumItem={LookupsImportEnum.position}
          onSave={() => {
            setFilter((items) => ({ ...items, page: items.page }));
          }}
          isOpenChanged={() => {
      ...
      {isOpenDeleteDialog && (
        <ConfirmDeleteDialog
          activeItem={activeItem}
          successMessage="position-setups-deleted-successfully"
          onSave={() => {
          ...
          descriptionMessage="position-setups-delete-description"
          deleteApi={DeleteSetupsPosition}
          apiProps={{
            uuid:
              (isBulkDelete
          ...
          errorMessage="position-setups-delete-failed"
          activeItemKey="uuid"
          apiDeleteKey="uuid"
          parentTranslationPath={parentTranslationPath}
