...
import { JobsTitlesManagementDialog } from './dialogs';
import { JobsTitlesPermissions } from '../../../../permissions';
import { LookupImportDialog } from '../../shared/dialogs/lookups-import/LookupsImport.Dialog';

const translationPath = 'JobsTitlesPage.';
const parentTranslationPath = 'SetupsPage';

const JobsTitlesPage = () => {
   ...
   * @Description this method is to get all jobs titles with filter
   */
  const getAllSetupsJobsTitles = useCallback(async () => {
    setIsLoading(true);
    const response = await GetAllSetupsJobsTitles({ ...filter });
    setIsLoading(false);
  ...
  useEffect(() => {
    void getAllSetupsJobsTitles();
  }, [getAllSetupsJobsTitles, filter]);

  return (
    <div className="positions-page page-wrapper">
      <div className="page-header-wrapper px-3 pb-3">
        <span className="header-text-x2 d-flex mb-1">
          {t(`${translationPath}jobs-titles`)}
        </span>
        <span className="description-text">
          {t(`${translationPath}jobs-titles-description`)}
        </span>
      </div>
          ...
          <div className="px-2">
            ...
            <div className="d-inline-flex mb-2">
              <TableColumnsPopoverComponent
                ...
                feature_name={TablesNameEnum.JobTitle.key}
            ...
            >
              ...
              <span className="px-2">
                {t(`${translationPath}import-${LookupsImportEnum.job_title.value}`)}
              </span>
            </ButtonBase>
            <ButtonBase
              disabled={
                !getIsAllowedPermissionV2({
                  permissionId: JobsTitlesPermissions.AddJobsTitles.key,
                  permissions: permissionsReducer,
                })
              }
              onClick={() => {
            ...
            >
              ...
              <span className="px-1">{t(`${translationPath}add-jobs-titles`)}</span>
            </ButtonBase>
          </div>
        </div>
        <div className="px-2">
          <TablesComponent
            ...
            data={jobsTitles.results}
            isLoading={isLoading || isLoadingColumns}
            headerData={tableColumns}
            pageIndex={filter.page - 1}
            pageSize={filter.limit}
            totalItems={jobsTitles.totalCount}
            isWithCheckAll
            isWithCheck
            isDynamicDate
            uniqueKeyInput="uuid"
            isSelectAllDisabled={
              !getIsAllowedPermissionV2({
                permissionId: JobsTitlesPermissions.DeleteJobsTitles.key,
                permissions: permissionsReducer,
              })
            }
            ...
            onSelectAllCheckboxChanged={() => {
              setSelectedRows((items) =>
                globalSelectedRowsHandler(
                  items,
                  jobsTitles.results.filter((item) => item.can_delete !== false),
                ),
              );
            }}
              ...
              getDisabledAction: (row, rowIndex, action) => {
                if (action.key === SystemActionsEnum.edit.key)
                  return !getIsAllowedPermissionV2({
                    permissionId: JobsTitlesPermissions.UpdateJobsTitles.key,
                    permissions: permissionsReducer,
                  });
      ...
      {isOpenLookupImportDialog && (
        <LookupImportDialog
          enumItem={LookupsImportEnum.job_title}
          onSave={() => {
            setFilter((items) => ({ ...items, page: items.page }));
          }}
          isOpenChanged={() => {
      ...
      {isOpenDeleteDialog && (
        <ConfirmDeleteDialog
          ...
          descriptionMessage="jobs-titles-delete-description"
          deleteApi={DeleteSetupsJobsTitles}
          apiProps={{
            uuid:
              (isBulkDelete
          ...
          errorMessage="job-title-delete-failed"
          activeItemKey="uuid"
          apiDeleteKey="uuid"
          parentTranslationPath={parentTranslationPath}
