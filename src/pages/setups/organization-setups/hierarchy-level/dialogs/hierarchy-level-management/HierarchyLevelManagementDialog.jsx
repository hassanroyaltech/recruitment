...
import {
  CreateSetupsHierarchyLevel,
  GetSetupsHierarchyLevelById,
  UpdateSetupsHierarchyLevel,
} from '../../../../../../services';
import { numericAndAlphabeticalAndSpecialExpression } from '../../../../../../utils';

export const HierarchyLevelManagementDialog = ({
  activeItem,
  isOpen,
  onSave,
  isOpenChanged,
  ...
  const getEditInit = useCallback(async () => {
    setIsLoading(true);
    const response = await GetSetupsHierarchyLevelById({
  ...
  const saveHandler = async (event) => {
    ...
    setIsLoading(true);
    let response;
    if (activeItem) response = await UpdateSetupsHierarchyLevel(state);
    else response = await CreateSetupsHierarchyLevel(state);
    setIsLoading(false);
    if (response && (response.status === 200 || response.status === 201)) {
      showSuccess(
        t(
          `${translationPath}${
            (activeItem && 'hierarchy-level-updated-successfully')
    ...
    //     t(
    //       `${translationPath}${
    //         (activeItem && 'hierarchy-level-update-failed')
    //         || 'hierarchy-level-create-failed'
    //       }`,
    //     ),
    //     response,
    //   );
  ...
  return (
    <DialogComponent
      maxWidth="md"
      titleText={(activeItem && 'edit-hierarchy-level') || 'add-hierarchy-level'}
      dialogContent={
...
HierarchyLevelManagementDialog.propTypes = {
...
HierarchyLevelManagementDialog.defaultProps = {
