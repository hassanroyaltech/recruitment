...
export const OrganizationGroupManagementDialog = ({
  ...
  const getEditInit = useCallback(async () => {
    setIsLoading(true);
    const response = await getSetupsOrganizationGroupById({
  ...
  const saveHandler = async (event) => {
    ...
    setIsLoading(true);
    let response;
    if (activeItem) response = await UpdateSetupsOrganizationGroup(state);
    else response = await CreateSetupsOrganizationGroup(state);
    setIsLoading(false);
    if (
      response
      ...
      showSuccess(
        t(
          `${translationPath}${
            (activeItem && 'business-group-updated-successfully')
            || 'business-group-created-successfully'
          }`,
        ),
      );
      if (onSave) onSave();
      ...
      showError(
        t(
          `${translationPath}${
            (activeItem && 'business-group-update-failed')
            || 'business-group-create-failed'
          }`,
        ),
        response,
      );
  ...
  return (
    <DialogComponent
      maxWidth="sm"
      titleText={(activeItem && 'edit-business-group') || 'add-new-business-group'}
      dialogContent={
...
OrganizationGroupManagementDialog.propTypes = {
...
OrganizationGroupManagementDialog.defaultProps = {
