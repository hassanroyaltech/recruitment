...
export const LocationsManagementDialog = ({
  activeItem,
  isOpen,
  onSave,
  isOpenChanged,
  ...
  const getEditInit = useCallback(async () => {
    setIsLoading(true);
    const response = await getSetupsLocationsById({
  ...
  const saveHandler = async (event) => {
    ...
    setIsLoading(true);
    let response;
    if (activeItem) response = await UpdateSetupsLocations(state);
    else response = await CreateSetupsLocations(state);
    setIsLoading(false);
    if (response && (response.status === 200 || response.status === 201)) {
      showSuccess(
        t(
          `${translationPath}${
            (activeItem && 'location-updated-successfully')
      ...
      showError(
        t(
          `${translationPath}${
            (activeItem && 'location-update-failed') || 'location-create-failed'
          }`,
        ),
        response,
      );
  ...
  return (
    <DialogComponent
      maxWidth="md"
      titleText={(activeItem && 'edit-location') || 'add-location'}
      contentClasses="px-0"
      dialogContent={
...
LocationsManagementDialog.propTypes = {
...
LocationsManagementDialog.defaultProps = {
  isOpenChanged: undefined,
  onSave: undefined,
  activeItem: undefined,
  translationPath: 'LocationsManagementDialog.',
};
