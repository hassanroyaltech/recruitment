...
import { JobTitleManagementDialog } from './dialogs';
import {
  LookupsImportEnum,
  SystemActionsEnum,
  TablesNameEnum,
} from '../../../../enums';
import { PositionTitlePermissions } from '../../../../permissions';
import { LookupImportDialog } from '../../shared/dialogs/lookups-import/LookupsImport.Dialog';

const translationPath = 'JobTitlePage.';
...
const JobTitlePage = () => {
   ...
   * @Description this method is to get all jobTitle with filter
   */
  const getAllCategories = useCallback(async () => {
    setIsLoading(true);
    const response = await GetAllSetupsPositionsTitle({ ...filter });
    setIsLoading(false);
  ...
  return (
    <div className="jobTitle-page page-wrapper">
          ...
          <div className="px-2">
            ...
            <div className="d-inline-flex mb-2">
              <TableColumnsPopoverComponent
                ...
                feature_name={TablesNameEnum.PositionTitle.key}
            ...
            >
              ...
              <span className="px-2">{t(`${translationPath}import-${LookupsImportEnum.position_title.value}`)}</span>
            </ButtonBase>
            <ButtonBase
              disabled={
                !getIsAllowedPermissionV2({
            ...
            >
              ...
              <span className="px-1">{t(`${translationPath}add-job-title`)}</span>
            </ButtonBase>
          </div>
        </div>
        <div className="px-2">
          <TablesComponent
            ...
            data={jobTitle.results}
            headerData={tableColumns}
            pageIndex={filter.page - 1}
            totalItems={jobTitle.totalCount}
            ...
            isSelectAllDisabled={
              !getIsAllowedPermissionV2({
                permissionId: PositionTitlePermissions.DeletePositionTitle.key,
                permissions: permissionsReducer,
              })
            }
            ...
            onSelectAllCheckboxChanged={() => {
              setSelectedRows((items) =>
                globalSelectedRowsHandler(
                  items,
                  jobTitle.results.filter((item) => item.can_delete !== false),
                ),
              );
            }}
              ...
              getDisabledAction: (row, rowIndex, action) => {
                if (action.key === SystemActionsEnum.edit.key)
                  return !getIsAllowedPermissionV2({
                    permissionId: PositionTitlePermissions.UpdatePositionTitle.key,
                    permissions: permissionsReducer,
                  });
      ...
      {isOpenLookupImportDialog && (
        <LookupImportDialog
          enumItem={LookupsImportEnum.position_title}
          onSave={() => {
            setFilter((items) => ({ ...items, page: items.page }));
          }}
          isOpenChanged={() => {
      ...
      {isOpenDeleteDialog && (
        <ConfirmDeleteDialog
          ...
          descriptionMessage="job-title-delete-description"
          deleteApi={DeleteSetupsPositionTitle}
          apiProps={{
            uuid:
              (isBulkDelete
