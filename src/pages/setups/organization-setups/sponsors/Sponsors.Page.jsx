...
import { DeleteSetupsSponsor, GetAllSetupsSponsors } from '../../../../services';
import {
...
import { SponsorsManagementDialog } from './dialogs';
import { SponsorsPermissions } from '../../../../permissions';
import { LookupImportDialog } from '../../shared/dialogs/lookups-import/LookupsImport.Dialog';

const translationPath = 'SponsorsPage.';
...
const SponsorsPage = () => {
   ...
   * @Description this method is to get all sponsors with filter
   */
  const getAllSetupsSponsors = useCallback(async () => {
    setIsLoading(true);
    const response = await GetAllSetupsSponsors({ ...filter });
    setIsLoading(false);
  ...
  }, [getAllSetupsSponsors, filter]);

  return (
    <div className="sponsors-page page-wrapper">
      <div className="page-header-wrapper px-3 pb-3">
        <span className="header-text-x2 d-flex mb-1">
          {t(`${translationPath}sponsors`)}
        </span>
        <span className="description-text">
          {t(`${translationPath}sponsors-setup-description`)}
        </span>
      </div>
          ...
          <div className="px-2">
            ...
            <div className="d-inline-flex mb-2">
              <TableColumnsPopoverComponent
                ...
                feature_name={TablesNameEnum.Sponsors.key}
            ...
            >
              ...
              <span className="px-2">{t(`${translationPath}import-${LookupsImportEnum.sponsors.value}`)}</span>
            </ButtonBase>
            <ButtonBase
              disabled={
                !getIsAllowedPermissionV2({
                  permissionId: SponsorsPermissions.AddSponsors.key,
                  permissions: permissionsReducer,
                })
              }
              onClick={() => {
            ...
            >
              ...
              <span className="px-1">{t(`${translationPath}add-sponsor`)}</span>
            </ButtonBase>
          </div>
        </div>
        <div className="px-2">
          <TablesComponent
            ...
            totalItems={sponsors.totalCount}
            ...
            isSelectAllDisabled={
              !getIsAllowedPermissionV2({
                permissionId: SponsorsPermissions.DeleteSponsors.key,
                permissions: permissionsReducer,
              })
            }
            ...
            onSelectAllCheckboxChanged={() => {
              setSelectedRows((items) =>
                globalSelectedRowsHandler(
                  items,
                  sponsors.results.filter((item) => item.can_delete !== false),
                ),
              );
            }}
              ...
              getDisabledAction: (row, rowIndex, action) => {
                if (action.key === SystemActionsEnum.edit.key)
                  return !getIsAllowedPermissionV2({
                    permissionId: SponsorsPermissions.UpdateSponsors.key,
                    permissions: permissionsReducer,
                  });
      ...
      {isOpenLookupImportDialog && (
        <LookupImportDialog
          enumItem={LookupsImportEnum.sponsors}
          onSave={() => {
            setFilter((items) => ({ ...items, page: items.page }));
          }}
          isOpenChanged={() => {
      ...
      {isOpenDeleteDialog && (
        <ConfirmDeleteDialog
          activeItem={activeItem}
          successMessage="sponsor-setups-deleted-successfully"
          onSave={() => {
          ...
          descriptionMessage="sponsor-setups-delete-description"
          deleteApi={DeleteSetupsSponsor}
          apiProps={{
            uuid:
              (isBulkDelete
          ...
          errorMessage="sponsor-setups-delete-failed"
          activeItemKey="uuid"
          apiDeleteKey="uuid"
          parentTranslationPath={parentTranslationPath}
