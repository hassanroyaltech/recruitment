...
export const SponsorsManagementDialog = ({
  ...
  const getEditInit = useCallback(async () => {
    setIsLoading(true);
    const response = await getSetupsSponsorsById({
  ...
  const saveHandler = async (event) => {
    ...
    setIsLoading(true);
    let response;
    if (activeItem) response = await UpdateSetupsSponsors(state);
    else response = await CreateSetupsSponsors(state);
    setIsLoading(false);
    if (response && (response.status === 200 || response.status === 201)) {
      showSuccess(
        t(
          `${translationPath}${
            (activeItem && 'sponsor-updated-successfully')
      ...
      showError(
        t(
          `${translationPath}${
            (activeItem && 'sponsor-update-failed') || 'sponsor-create-failed'
          }`,
        ),
        response,
      );
  ...
  return (
    <DialogComponent
      maxWidth="md"
      titleText={(activeItem && 'edit-sponsor') || 'add-sponsor'}
      contentClasses="px-0"
      dialogContent={
...
SponsorsManagementDialog.propTypes = {
...
SponsorsManagementDialog.defaultProps = {
  isOpenChanged: undefined,
  onSave: undefined,
  activeItem: undefined,
  translationPath: 'SponsorsManagementDialog.',
};
