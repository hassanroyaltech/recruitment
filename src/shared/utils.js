...
export const getTypeString = (type) => {
  ...
  if (type == '2') return 'video_assessment';
...
export const getRoleNameAlias = (roleName) => {
  switch (roleName) {
  case 'ats':
    return 'EVA-REC';
  case 'career-portal':
  ...
  case 'pipeline':
    return 'Setup Pipelines';
  case 'questionnaires':
    return 'Questionnaires';
  case 'requirements':
    return 'Application Requirements';
  case 'responsibility':
    return 'Manage User Responsibilities';
  case 'resumes-management-system':
    return 'Resume Management System';
  ...
  case 'template_assessment':
    return 'EVA-SSESS Templates';
  case 'template_ats':
    return 'EVA-REC Templates';
...
export const getOrder = (sections, section) =>
  sections.filter((o) => o.title === section)[0].order;
